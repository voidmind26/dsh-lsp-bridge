window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 程序来源与工作区无关：配置里写的、PATH 里的、目标工程本地的、插件装在前缀里的都算数。
    const COMMAND_SOURCE_TEXT = { plugin: '插件安装', project: '工程本地', path: 'PATH 中的程序' };
    ...
    function selectedServers(report, choices = {}) {
      return (report.servers || []).flatMap(entry => {
        ...
        // 与 Host 侧同一语义：项目根不落盘。扫描到的根只是当前会话的事实，写进配置会固化成
        // 绝对路径（换机器即失效），还会把多仓工作区的无 file 查询缩小到一个仓。
        // 显式配置的 roots 原样保留；`roots: []` 用于清掉旧的固定根（字段级合并会保留省略的字段）。
        return [{ id: entry.serverId, command, args: entry.args, languages: entry.languages, rootMarkers: entry.rootMarkers, roots: entry.targetSource === 'config' ? [...(entry.roots || [])] : [], ...(entry.initializationOptions === undefined ? {} : { initializationOptions: entry.initializationOptions }) }];
      });
    }
